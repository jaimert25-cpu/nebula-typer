...
// `enemies` es el arreglo vivo compartido. makeEnemy y Director reciben
// level/diff como parametros (inyeccion de dependencias) para no depender
// de game.js. Los power-ups son un tipo mas que trae su efecto (e.power).
// =====================================================================
...
export function makeEnemy(typeKey, level, diff){
  ...
  // intentar que la primera letra sea unica (no aplica a power-ups)
  for (let k = 0; k < 6 && typeKey !== 'power' && enemies.some(e => !e.dead && e.word[0] === word[0]); k++){
  ...
  return {
    type: typeKey, T, word, power, typed: 0, mistakes: 0, x, y: -30, speed,
...
export const Director = (() => {
  ...
  function pickType(level){
    ...
    if (Math.random() < 0.05) return 'power';
    const r = Math.random();
